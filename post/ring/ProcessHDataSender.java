...
public class ProcessHDataSender implements Runnable {
    ...
    ProcessHDataSender(Socket nextSocket, Socket managerSocket){
    ...
    public void run() {
        ...
        // TODO
        destination = ProcessH.getDestination();                            // 这里实际上是获取一个目的地址，因为数据内部的东西可以临时生成，我们只需要一个目的地址即可
        ProcessG.flushHasData();                                            // 刷新状态位
        // 初始化数据帧
        frame.isToken = false;
        frame.destinationHost = destination;
        // TODO 修改Hosts
        frame.sourceHost = ProcessH.getHostID();
        frame.content = "Host 8 " + frame.sourceHost + ": Hello World!";
