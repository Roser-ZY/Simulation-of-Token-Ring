...
public class ProcessHMessageListener implements Runnable {
    ...
    ProcessHMessageListener(Socket managerSocket, Socket nextSocket){
                ...
                switch (MESSAGE) {
                    ...
                    case SET_ID: {
                        if (!GlobalStatus.getHasSetHostID()) {
                            ...
                            try{
                                ...
                                ProcessH.setHostIDs(hostIDs);
                            }catch (Exception e){
                                e.printStackTrace();
                            }
                            // TODO
                            ProcessH.setHostID(hostIDs[7]);
                            System.out.println(MESSAGE);
                        }
                    }break;
                    // TODO
                    case CHOSEN_AM: {
                        System.out.println("Host 8: Chosen as AM.");
                        ProcessH.setIsAM();
                    }break;
                    case START_RING: {
                        if (!GlobalStatus.getHasStarted()) {                                                    // 如果未开始，则开始，如果已开始，则无视
                            ...
                            // 新建一个生成数据的线程
                            // TODO
                            new Thread(new ProcessHDataGeneration()).start();
                    ...
                    case RESET_RING: {
                        if (GlobalStatus.getHasStarted()) {
                            ...
                            GlobalStatus.reset();
                            // TODO
                            ProcessHDataGeneration.setStatus();
                            System.out.println(MESSAGE);
                            // TODO
                            if(ProcessH.ifIsAM()) {
                                // 设置标志位
                                ProcessH.reset();
                                System.out.println("Host 8: Reset " + ProcessH.getIfReset());
                            }
                        }
                        break;
                    }
