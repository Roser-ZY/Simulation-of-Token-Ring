...
public class ProcessGDataListener implements Runnable {
    ...
    // 模拟未接收的情况
    private Random random = new Random(((new Date()).getTime() % 5343));

    ProcessGDataListener(Socket socket, Socket nextSocket, Socket managerSocket){
        this.socket = socket;
        ...
        while (true) {
            try {
                ...
                if(frame.isToken){
                    ...
                    // 如果到达源主机，则判断是否已被接受
                    if(frame.sourceHost.equals(ProcessG.getHostID())){
                        if(frame.isReceived){
                            // TODO
                    ...
                    // 如果到达目的主机，则接收
                    else if(frame.destinationHost.equals(ProcessG.getHostID()) && random.nextInt(50) != 34){
                        frame.isReceived = true;
                        ProcessG.copyFrame(frame);
