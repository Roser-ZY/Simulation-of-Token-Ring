...
public class ProcessB {
    ...
    //    private static Boolean hasReceivedToken = false;                            // 判断是否收到令牌，只有截获令牌并且有数据要发送才可以发送自己的数据
...
//    private static int frameTimes;                                            // 帧经过次数，令牌或者数据，用于监测站
    ...
    public static void main(String[] args) {
        // 建立服务器
        try {
            serverSocket = new ServerSocket(9002);                                  // 当前主机使用端口9002
        }catch (Exception e){
            e.printStackTrace();
        }

        // 接收前一个主机的Socket
        // 这里我们刚创建serverSocket就开始接收Socket，目的就是为了尽快接收，然后建立与下一台主机的连接，这样形成一条链，很顺畅的就打开并连接了所有的主机
        // 可以思考一下如果不是这样会怎样死锁，或者还有没有别的顺序和方法防止死锁
        try {
        ...
        while(!clientForProcessStatus) {
            connectNextProcess(9003);                                               // 下一台主机使用端口9003
            try{
                Thread.sleep(1000);
            }catch (Exception e){
                e.printStackTrace();
        ...
        // TODO
        System.out.println("成功连接主机3！");

        //建立连接Manager的客户端
        while(!clientForManagerStatus){
            connectManagerProcess(10002);                                           // 连接主进程的10002端口
            try{
                Thread.sleep(1000);
            }catch (Exception e){
                e.printStackTrace();
        ...
        new Thread(new ProcessBMessageListener(clientForManagerSocket, clientForProcessSocket)).start();            // 监听来自Manager的消息
        new Thread(new ProcessBDataListener(preSocket, clientForProcessSocket, clientForManagerSocket)).start();    // 监听前一台主机发来的数据，第一个为当前主机获取的前一台主机的Socket，第二个为当前主机对下一台主机的客户Socket，第三个为当前主机对主进程的客户Socket
