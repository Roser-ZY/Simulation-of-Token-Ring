...
public class ProcessDDataListener implements Runnable {
    ...
    // 模拟未接收的情况
    private Random random = new Random(((new Date()).getTime() % 9238));

    ProcessDDataListener(Socket socket, Socket nextSocket, Socket managerSocket){
        this.socket = socket;
    ...
    public void run() {
        while (true) {
            try {
                ...
                if(frame.isToken){
                    // TODO
                    System.out.println("Host4: Get Token!");
                    // 判断是否reset
                    if(ProcessD.getIfReset()){
                        // TODO
                        System.out.println("Reset!");
                        // 重置状态位
                        ProcessD.resetIfReset();
                        // 通知主进程
                        new Thread(new ProcessMessageSender(managerSocket, Message.RESET_COMPLETE)).start();
                    }
                    // 如果有数据，则将令牌截获，并发送数据
                    else if(ProcessD.ifHasData()){
                        new Thread(new ProcessMessageSender(managerSocket, Message.TOKEN_RECEIVED)).start();
                        // 创建发送数据的线程
                        new Thread(new ProcessDDataSender(nextSocket, managerSocket)).start();
                    }
                    // 否则将令牌转发
                    else{
                    ...
                    // 如果到达源主机，则判断是否已被接受
                    if(frame.sourceHost.equals(ProcessD.getHostID())){
                        if(frame.isReceived){
                            // TODO
                    ...
                    // 如果到达目的主机，则接收
                    else if(frame.destinationHost.equals(ProcessD.getHostID()) && random.nextInt(50) != 7){
                        frame.isReceived = true;
                        ProcessD.copyFrame(frame);
