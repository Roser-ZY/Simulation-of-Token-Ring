...
public class ProcessG {
    ...
    public static void main(String[] args) {
        // 建立服务器
        try {
            serverSocket = new ServerSocket(9007);                                  // 当前主机使用端口9007
        }catch (Exception e){
            e.printStackTrace();
        }
        ...
        while(!clientForProcessStatus) {
            connectNextProcess(9008);                                               // 下一台主机使用端口9008
            try{
                Thread.sleep(1000);
            }catch (Exception e){
                e.printStackTrace();
        ...
        // TODO
        System.out.println("成功连接主机8！");

        // 建立连接Manager的客户端
        while(!clientForManagerStatus){
            connectManagerProcess(10007);                                           // 连接主进程的10007端口
            try{
                Thread.sleep(1000);
            }catch (Exception e){
                e.printStackTrace();
        ...
        new Thread(new ProcessGMessageListener(clientForManagerSocket, clientForProcessSocket)).start();            // 监听来自Manager的消息
        new Thread(new ProcessGDataListener(preSocket, clientForProcessSocket, clientForManagerSocket)).start();    // 监听前一台主机发来的数据，第一个为当前主机获取的前一台主机的Socket，第二个为当前主机对下一台主机的客户Socket，第三个为当前主机对主进程的客户Socket
