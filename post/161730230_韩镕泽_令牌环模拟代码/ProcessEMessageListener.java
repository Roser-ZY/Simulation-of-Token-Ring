...
public class ProcessEMessageListener implements Runnable {
    ...
    ProcessEMessageListener(Socket managerSocket, Socket nextSocket){
                ...
                switch (MESSAGE) {
                    ...
                    case SET_ID: {
                        if (!GlobalStatus.getHasSetHostID()) {
                            ...
                            try{
                                ...
                                ProcessE.setHostIDs(hostIDs);
                            }catch (Exception e){
                                e.printStackTrace();
                            }
                            // TODO
                            ProcessE.setHostID(hostIDs[4]);
                            System.out.println(MESSAGE);
                        }
                    }break;
                    // TODO
                    case CHOSEN_AM: {
                        System.out.println("Host 5: Chosen as AM.");
                        ProcessE.setIsAM();
                    }break;
                    case START_RING: {
                        if (!GlobalStatus.getHasStarted()) {                                                    // 如果未开始，则开始，如果已开始，则无视
                            ...
                            // 新建一个生成数据的线程
                            // TODO
                            new Thread(new ProcessEDataGeneration()).start();
                    ...
                    case RESET_RING: {
                        if (GlobalStatus.getHasStarted()) {
                            ...
                            GlobalStatus.reset();
                            // TODO
                            ProcessEDataGeneration.setStatus();
                            System.out.println(MESSAGE);
                            // TODO
                            if(ProcessE.ifIsAM()) {
                                // 设置标志位
                                ProcessE.reset();
                                System.out.println("Host 5: Reset " + ProcessE.getIfReset());
                            }
                        }
                        break;
                    }
