...
public class ProcessFMessageListener implements Runnable{
    ...
    ProcessFMessageListener(Socket managerSocket, Socket nextSocket){
                ...
                switch (MESSAGE) {
                    ...
                    case SET_ID: {
                        if (!GlobalStatus.getHasSetHostID()) {
                            ...
                            try{
                                ...
                                ProcessF.setHostIDs(hostIDs);
                            }catch (Exception e){
                                e.printStackTrace();
                            }
                            // TODO
                            ProcessF.setHostID(hostIDs[5]);
                            System.out.println(MESSAGE);
                        }
                    }break;
                    // TODO
                    case CHOSEN_AM: {
                        System.out.println("Host 6: Chosen as AM.");
                        ProcessF.setIsAM();
                    }break;
                    case START_RING: {
                        if (!GlobalStatus.getHasStarted()) {                                                    // 如果未开始，则开始，如果已开始，则无视
                            ...
                            // 新建一个生成数据的线程
                            // TODO
                            new Thread(new ProcessFDataGeneration()).start();
                    ...
                    case RESET_RING: {
                        if (GlobalStatus.getHasStarted()) {
                            ...
                            GlobalStatus.reset();
                            // TODO
                            ProcessFDataGeneration.setStatus();
                            System.out.println(MESSAGE);
                            // TODO
                            if(ProcessF.ifIsAM()) {
                                // 设置标志位
                                ProcessF.reset();
                                System.out.println("Host 6: Reset " + ProcessF.getIfReset());
                            }
                        }
                        break;
                    }
