...
public class ProcessDMessageListener implements Runnable {
    ...
    ProcessDMessageListener(Socket managerSocket, Socket nextSocket){
                ...
                switch (MESSAGE) {
                    ...
                    case SET_ID: {
                        if (!GlobalStatus.getHasSetHostID()) {
                            ...
                            try{
                                ...
                                ProcessD.setHostIDs(hostIDs);
                            }catch (Exception e){
                                e.printStackTrace();
                            }
                            // TODO
                            ProcessD.setHostID(hostIDs[3]);
                            System.out.println(MESSAGE);
                        }
                    }break;
                    // TODO
                    case CHOSEN_AM: {
                        System.out.println("Host 4: Chosen as AM.");
                        ProcessD.setIsAM();
                    }break;
                    case START_RING: {
                        if (!GlobalStatus.getHasStarted()) {                                                    // 如果未开始，则开始，如果已开始，则无视
                            ...
                            // 新建一个生成数据的线程
                            // TODO
                            new Thread(new ProcessDDataGeneration()).start();
                    ...
                    case RESET_RING: {
                        if (GlobalStatus.getHasStarted()) {
                            ...
                            GlobalStatus.reset();
                            // TODO
                            ProcessDDataGeneration.setStatus();
                            System.out.println(MESSAGE);
                            // TODO
                            if(ProcessD.ifIsAM()) {
                                // 设置标志位
                                ProcessD.reset();
                                System.out.println("Host 4: Reset " + ProcessD.getIfReset());
                            }
                        }
                        break;
                    }
