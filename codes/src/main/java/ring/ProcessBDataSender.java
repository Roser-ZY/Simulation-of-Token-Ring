...
public class ProcessBDataSender implements Runnable {
    ...
    ProcessBDataSender(Socket nextSocket, Socket managerSocket){
    ...
    public void run() {
        ...
        destination = ProcessB.getDestination();                            // 这里实际上是获取一个目的地址，因为数据内部的东西可以临时生成，我们只需要一个目的地址即可
        ProcessB.flushHasData();                                            // 刷新状态位
        // 初始化数据帧
        frame.isToken = false;
        frame.destinationHost = destination;
        frame.sourceHost = ProcessB.getHostID();
        frame.content = "Host 2 " + frame.sourceHost + ": Hello World!";
