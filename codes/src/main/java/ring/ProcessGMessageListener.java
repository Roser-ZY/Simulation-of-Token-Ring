...
public class ProcessGMessageListener implements Runnable {
    ...
    ProcessGMessageListener(Socket managerSocket, Socket nextSocket){
                ...
                switch (MESSAGE) {
                    ...
                    case SET_ID: {
                        if (!GlobalStatus.getHasSetHostID()) {
                            ...
                            try{
                                ...
                                ProcessG.setHostIDs(hostIDs);
                            }catch (Exception e){
                                e.printStackTrace();
                            }
                            // TODO
                            ProcessG.setHostID(hostIDs[6]);
                            System.out.println(MESSAGE);
                        }
                    }break;
                    // TODO
                    case CHOSEN_AM: {
                        System.out.println("Host 7: Chosen as AM.");
                        ProcessG.setIsAM();
                    }break;
                    case START_RING: {
                        if (!GlobalStatus.getHasStarted()) {                                                    // 如果未开始，则开始，如果已开始，则无视
                            ...
                            // 新建一个生成数据的线程
                            // TODO
                            new Thread(new ProcessGDataGeneration()).start();
                    ...
                    case RESET_RING: {
                        if (GlobalStatus.getHasStarted()) {
                            ...
                            GlobalStatus.reset();
                            // TODO
                            ProcessGDataGeneration.setStatus();
                            System.out.println(MESSAGE);
                            // TODO
                            if(ProcessG.ifIsAM()) {
                                // 设置标志位
                                ProcessG.reset();
                                System.out.println("Host 7: Reset " + ProcessG.getIfReset());
                            }
                        }
                        break;
                    }
